...
export class SearchPage extends React.Component {
  ...
  toggleSortingOrder(order) {
    ...
    if (order === 'release') {
      this.state.movies.sort((m1, m2) => m2.release_year - m1.release_year)
    } else {
      this.state.movies.sort((m1, m2) => m2.rating - m1.rating)
    }
  }
  ...
  onSearch() {
    if (this.input.value && this.input.value.trim()) {
      this.props.history.push(`/search/${this.input.value.trim()}`)
      this.setState({
        movies: [
          {
            "unit": 84,
    ...
    return (
      <div>
        <header>
          ...
          <input id="search" type="text" defaultValue={this.props.match.params.query} ref={input => this.input = input} onKeyPress={this.onEnterButtonPress.bind(this)} />
          <div className={s.row}>
            <p>search by</p>
            ...
            <div type className={`${s.button} ${s.searchButton}`} onClick={this.onSearch.bind(this)}>search</div>
          </div>
        </header>
        ...
        {this.state.movies.length ?
          <div className={s.container}>
            {this.state.movies.map(item => <Link to={`/film/${item.show_title}`} key={item.show_id}><FilmCard key={item.show_id} movie={item} /></Link>)}
          </div> :
